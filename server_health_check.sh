...
# set -o pipefail: if any command in a pipeline fails, trait the whole pipeline as failed
set -euo pipefail
# --- Global Constants ---
# We use uppercase names by convention for constants
LOG_FILE=$(mktemp /tmp/server_health.XXXXXX)
readonly LOG_FILE
# 'readonly' means this value can't be changed
...
check_server() {
	...
	# Usae an SSH here-document to send a batch of commands in one connection
...
# NR==2 means print only the second line of df ouput
